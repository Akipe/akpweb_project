...
clean:  ## Clear all caches
	rm -R ./docker/php-cli/cache.composer/*
	rm -R ./docker/php-cli/cache.composer/.*

shell: ## Execute command from PHP container with 
	sudo docker exec -it -u 1000:1000 akpweb_php-cli_dev $(COMMAND)
...
composer-install:
	sudo docker exec -it -u 1000:1000 akpweb_php-cli_dev composer install

composer-update:
	sudo docker exec -it -u 1000:1000 akpweb_php-cli_dev composer update

gpm:
	sudo docker exec -it -u 1000:1000 akpweb_php-cli_dev bin/gpm $(COM)

gpm-update:
	sudo docker exec -it -u 1000:1000 akpweb_php-cli_dev bin/gpm selfupgrade -f
